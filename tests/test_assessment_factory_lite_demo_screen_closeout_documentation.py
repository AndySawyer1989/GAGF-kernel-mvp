...
DOC_PATH = Path("docs/ASSESSMENT_FACTORY_LITE_DEMO_SCREEN_CLOSEOUT.md")


def test_assessment_factory_lite_demo_screen_closeout_document_exists():
    assert DOC_PATH.exists()


def test_assessment_factory_lite_demo_screen_closeout_document_names_release_marker():
    content = DOC_PATH.read_text(encoding="utf-8")

    assert "Release 1.3.0" in content
    assert "assessment-factory-lite-demo-screen" in content
    assert "Sprint" in content
    assert "4.2" in content
    assert "complete" in content


def test_assessment_factory_lite_demo_screen_closeout_document_names_capability_chain():
    content = DOC_PATH.read_text(encoding="utf-8")
    ...
    assert "Demo UI HTML API" in content
    assert "Demo Screen Release Marker" in content


def test_assessment_factory_lite_demo_screen_closeout_document_names_service_and_endpoint():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_screen_closeout_document_names_html_shell_and_sections():
    content = DOC_PATH.read_text(encoding="utf-8")
    ...
    assert "Demo Safety Warnings" in content
    assert "Operator Demo Cards" in content
    assert "Buyer-Facing Export Preview" in content
    assert "Operator Actions" in content


def test_assessment_factory_lite_demo_screen_closeout_document_names_cards_and_warnings():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_screen_closeout_document_names_operator_actions():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_screen_closeout_document_names_demo_boundary():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_screen_closeout_document_names_next_track():
    content = DOC_PATH.read_text(encoding="utf-8")

    assert "Assessment Factory Lite Demo Sample Data Loader" in content
    assert "US-174 Assessment Factory Lite Demo Sample Rows Service" in content
    assert "US-175 Assessment Factory Lite Demo Sample Rows Endpoint" in content
    assert "US-176 Assessment Factory Lite Demo Sample Rows Documentation" in content
    assert "US-177 Assessment Factory Lite Demo Screen Sample Loader Integration" in content
    assert "US-178 Assessment Factory Lite Demo Screen Loader Release Marker" in content


def test_assessment_factory_lite_demo_screen_closeout_document_preserves_boundaries():
    content = DOC_PATH.read_text(encoding="utf-8")

    assert (
        "The Assessment Factory Lite Demo Screen does not certify products "
