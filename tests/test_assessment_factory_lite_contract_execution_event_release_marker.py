...
def test_assessment_factory_lite_contract_execution_event_release_marker_preserves_version_endpoint():
...
def test_assessment_factory_lite_contract_execution_event_release_marker_preserves_endpoint_route():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/contract-execution-event" in actual_routes


def test_assessment_factory_lite_contract_execution_event_release_marker_matches_service_contract():
...
def test_assessment_factory_lite_contract_execution_event_release_marker_matches_endpoint_contract():
...
def test_assessment_factory_lite_contract_execution_event_release_marker_preserves_source_layering():
    result = build_executed_event()

    assert result["source_contract_execution_review"]["release"] == (
        "assessment-factory-lite-scope-call-conversion"
    )
    assert result["source_contract_execution_review"]["version"] == "2.3.0"
    assert result["source_contract_execution_review"]["review_status"] == (
...
def test_assessment_factory_lite_contract_execution_event_release_marker_preserves_execution_and_downstream_boundaries():
    result = build_executed_event()

    assert result["execution_evidence"]["contract_execution_confirmed"] is True
    assert result["execution_evidence"]["executed_contract_reference_recorded"] is True
    assert result["execution_evidence"]["executed_at_recorded"] is True
    assert result["execution_evidence"]["execution_method_recorded"] is True
    assert result["execution_evidence"]["contract_executed"] is True

    assert result["signature_record"]["buyer_signed"] is True
    assert result["signature_record"]["provider_signed"] is True
    assert result["signature_record"]["signature_evidence_recorded"] is True
    assert result["signature_record"]["all_required_signatures_recorded"] is True
    assert result["signature_record"]["signature_record_is_not_invoice"] is True
    assert result["signature_record"]["signature_record_is_not_payment"] is True

    assert result["operator_confirmation"]["human_operator_confirmed_execution"] is True
    assert result["operator_confirmation"]["invoice_creation_approved"] is False
    assert result["operator_confirmation"]["payment_request_approved"] is False
