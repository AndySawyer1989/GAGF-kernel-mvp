...
def build_ready_review():
    return AssessmentFactoryLitePaymentRequestReviewService().build_review(
...
def test_assessment_factory_lite_payment_request_review_service_builds_contract():
    result = build_ready_review()

    assert result["status"] == "ok"
    assert result["event_type"] == "assessment_factory_lite_payment_request_review"
    ...
    assert result["review_stage"] == "payment_request_review"
    assert result["review_status"] == "ready_for_payment_request"
    assert result["payment_request_review_id"] == "payment-request-review-001"
    assert result["prepared_at"] == "2026-07-31T12:00:00+00:00"
    assert result["recommended_action"] == "prepare_payment_request_event"


def test_assessment_factory_lite_payment_request_review_service_summarizes_source_event():
    result = build_ready_review()

    assert result["source_invoice_creation_event"] == {
...
def test_assessment_factory_lite_payment_request_review_service_tracks_payment_request_details():
    result = build_ready_review()

    assert result["payment_request_details"] == {
        "payment_amount_confirmed": True,
        "invoice_reference_confirmed": True,
        "payment_due_date_confirmed": True,
        "payment_request_language_reviewed": True,
...
def test_assessment_factory_lite_payment_request_review_service_tracks_buyer_notice_readiness():
    result = build_ready_review()

    assert result["buyer_notice_readiness"] == {
        "buyer_notice_prepared": True,
        "buyer_notice_channel_confirmed": True,
        "payment_instructions_included": True,
...
def test_assessment_factory_lite_payment_request_review_service_tracks_operator_review_without_payment_request():
    result = build_ready_review()
...
def test_assessment_factory_lite_payment_request_review_service_builds_checklist_and_score():
    result = build_ready_review()
...
def test_assessment_factory_lite_payment_request_review_service_preserves_boundaries():
    result = build_ready_review()
    ...
    assert "payment_request_review_notes" in result["evidence_boundary"]["allowed_evidence"]
...
def test_assessment_factory_lite_payment_request_review_service_includes_audit_next_action_and_message():
    result = build_ready_review()
...
def test_assessment_factory_lite_payment_request_review_service_handles_pending_states():
    no_details = dict(PAYMENT_REQUEST_CONTEXT)
    no_details["payment_amount_confirmed"] = False

    result = AssessmentFactoryLitePaymentRequestReviewService().build_review(
        operator_approval=APPROVAL,
        event_context=EVENT_CONTEXT,
        follow_up_context=INTERESTED_CONTEXT,
        follow_up_event_context=FOLLOW_UP_EVENT_CONTEXT,
        ...
        invoice_context=INVOICE_CONTEXT,
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=no_details,
    )

    assert result["review_status"] == "pending_payment_request_details_review"
    assert "payment_request_details_ready" in result["review_blockers"]
    assert result["next_action"]["action"] == "complete_payment_request_details_review"

    no_notice = dict(PAYMENT_REQUEST_CONTEXT)
    no_notice["buyer_notice_prepared"] = False
    ...
    result = AssessmentFactoryLitePaymentRequestReviewService().build_review(
        ...
        invoice_context=INVOICE_CONTEXT,
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=no_notice,
    )

    assert result["review_status"] == "pending_buyer_notice_readiness"
    assert "buyer_notice_ready" in result["review_blockers"]
    assert result["next_action"]["action"] == "confirm_buyer_notice_readiness"

    no_operator = dict(PAYMENT_REQUEST_CONTEXT)
    no_operator["payment_request_reviewed_by_operator"] = False
    ...
    result = AssessmentFactoryLitePaymentRequestReviewService().build_review(
        ...
        invoice_context=INVOICE_CONTEXT,
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=no_operator,
    )

    assert result["review_status"] == "pending_operator_review"
    assert "payment_request_reviewed_by_operator" in result["review_blockers"]
    assert result["next_action"]["action"] == "complete_operator_payment_request_review"


def test_assessment_factory_lite_payment_request_review_service_blocks_invalid_upstream_export():
    ...
    assert result["source_invoice_creation_event"]["event_status"] == "blocked"
    assert result["recommended_action"] == "resolve_payment_request_review_gaps"