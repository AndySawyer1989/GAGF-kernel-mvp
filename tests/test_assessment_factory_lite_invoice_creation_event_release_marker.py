...
def test_assessment_factory_lite_invoice_creation_event_release_marker_preserves_version_endpoint():
...
def test_assessment_factory_lite_invoice_creation_event_release_marker_preserves_endpoint_route():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/invoice-creation-event" in actual_routes


def test_assessment_factory_lite_invoice_creation_event_release_marker_matches_service_contract():
...
def test_assessment_factory_lite_invoice_creation_event_release_marker_matches_endpoint_contract():
...
def test_assessment_factory_lite_invoice_creation_event_release_marker_preserves_source_layering():
    result = build_created_event()

    assert result["source_invoice_creation_review"]["release"] == (
        "assessment-factory-lite-scope-call-conversion"
    )
    assert result["source_invoice_creation_review"]["version"] == "2.3.0"
    assert result["source_invoice_creation_review"]["review_status"] == (
...
def test_assessment_factory_lite_invoice_creation_event_release_marker_preserves_invoice_and_downstream_boundaries():
    result = build_created_event()

    assert result["invoice_record"]["invoice_created"] is True
    assert result["invoice_record"]["invoice_reference_recorded"] is True
    assert result["invoice_record"]["invoice_created_at_recorded"] is True
    assert result["invoice_record"]["invoice_amount_recorded"] is True
    assert result["invoice_record"]["invoice_record_is_not_payment_request"] is True
    assert result["invoice_record"]["invoice_record_is_not_payment_confirmation"] is True
    ...
    assert result["delivery_record"]["invoice_delivery_is_not_payment_request"] is True

    assert result["operator_confirmation"]["human_operator_confirmed_invoice_creation"] is True
    assert result["operator_confirmation"]["payment_requested"] is False
