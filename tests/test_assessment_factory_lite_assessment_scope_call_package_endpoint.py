...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_returns_contract():
    ...
    assert payload["package_stage"] == "assessment_scope_call_package"
    assert payload["package_status"] == "ready"
    assert payload["package_id"] == "assessment-scope-call-package-001"
    assert payload["created_at"] == "2026-07-21T12:30:00+00:00"
    assert payload["recommended_action"] == "review_assessment_scope_call_package"


def test_assessment_factory_lite_assessment_scope_call_package_endpoint_includes_source_event_and_readiness():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
    ...
    assert payload["source_follow_up_event_record"] == {
    ...
    assert payload["scope_call_readiness"] == {
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_includes_agenda_and_boundaries():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
    ...
    payload = response.json()
    agenda_items = [item["item"] for item in payload["scope_call_agenda"]]
    ...
    assert all(item["required"] is True for item in payload["scope_call_agenda"])
    assert all(item["owner"] == "operator" for item in payload["scope_call_agenda"])
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_includes_approval_and_scheduling_boundaries():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
    ...
    assert payload["operator_approval_gate"] == {
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_includes_checklist_and_no_blockers_when_ready():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
    ...
    payload = response.json()
    checks = {item["check"]: item for item in payload["package_checklist"]}
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_includes_audit_next_action_and_message():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_review_required_for_questions_response():
    response = client.post(
        "/products/assessment-factory-lite/assessment-scope-call-package",
        json={
            "operator_approval": APPROVAL,
    ...
    assert payload["package_status"] == "review_required"
    assert payload["recommended_action"] == "resolve_assessment_scope_call_package_gaps"
    assert payload["scope_call_readiness"]["event_recorded"] is True
    assert payload["scope_call_readiness"]["buyer_interested"] is False
    assert "buyer_interested" in payload["package_blockers"]
    assert "scope_call_action_supported" in payload["package_blockers"]
    assert payload["next_action"] == {
...
def test_assessment_factory_lite_assessment_scope_call_package_endpoint_blocks_invalid_follow_up_event_and_preserves_route():
    ...
    assert payload["recommended_action"] == "resolve_assessment_scope_call_package_gaps"
    assert payload["source_follow_up_event_record"]["event_status"] == "blocked"
    assert "follow_up_event_recorded" in payload["package_blockers"]
    assert "buyer_interested" in payload["package_blockers"]
    assert "scope_call_action_supported" in payload["package_blockers"]
    assert payload["next_action"] == {
