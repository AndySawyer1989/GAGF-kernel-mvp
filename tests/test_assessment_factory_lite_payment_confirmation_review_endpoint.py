...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_builds_contract():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
    ...
    assert payload["event_type"] == "assessment_factory_lite_payment_confirmation_review"
    ...
    assert payload["review_stage"] == "payment_confirmation_review"
    assert payload["review_status"] == "ready_for_payment_confirmation"
    assert payload["payment_confirmation_review_id"] == "payment-confirmation-review-001"
    assert payload["prepared_at"] == "2026-08-02T12:00:00+00:00"
    assert payload["recommended_action"] == "prepare_payment_confirmation_event"


def test_assessment_factory_lite_payment_confirmation_review_endpoint_summarizes_source_event():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
    ...
    assert payload["source_payment_request_event"] == {
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_tracks_payment_evidence_review():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
    ...
    assert payload["payment_evidence_review"] == {
        "payment_receipt_available": True,
        "payment_reference_available": True,
        "received_amount_reviewed": True,
        "received_at_reviewed": True,
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_tracks_reconciliation_review():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
    ...
    assert payload["reconciliation_review"] == {
        "amount_matches_request": True,
        "invoice_reference_matches": True,
        "payment_method_reviewed": True,
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_tracks_operator_review_without_confirmation_or_authorization():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_builds_checklist_and_score():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_preserves_boundaries():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
    ...
    assert "payment_receipt_reference" in payload["evidence_boundary"]["allowed_evidence"]
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_includes_audit_next_action_and_message():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-review",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_review_endpoint_handles_pending_states():
    no_evidence = ready_payload()
    no_evidence["payment_confirmation_context"] = dict(PAYMENT_CONFIRMATION_CONTEXT)
    no_evidence["payment_confirmation_context"]["payment_receipt_available"] = False
    ...
    assert payload["review_status"] == "pending_payment_evidence_review"
    assert "payment_evidence_ready" in payload["review_blockers"]
    assert payload["next_action"]["action"] == "complete_payment_evidence_review"

    no_reconciliation = ready_payload()
    no_reconciliation["payment_confirmation_context"] = dict(PAYMENT_CONFIRMATION_CONTEXT)
    no_reconciliation["payment_confirmation_context"]["amount_matches_request"] = False
    ...
    assert payload["review_status"] == "pending_reconciliation_review"
    assert "reconciliation_ready" in payload["review_blockers"]
    assert payload["next_action"]["action"] == "complete_payment_reconciliation_review"

    no_operator = ready_payload()
    no_operator["payment_confirmation_context"] = dict(PAYMENT_CONFIRMATION_CONTEXT)
    no_operator["payment_confirmation_context"][
    ...
    assert "payment_confirmation_reviewed_by_operator" in payload["review_blockers"]
    assert payload["next_action"]["action"] == "complete_operator_payment_confirmation_review"


def test_assessment_factory_lite_payment_confirmation_review_endpoint_blocks_invalid_upstream_export():
    ...
    assert payload["source_payment_request_event"]["event_status"] == "blocked"
    assert payload["recommended_action"] == "resolve_payment_confirmation_review_gaps"


def test_assessment_factory_lite_payment_confirmation_review_endpoint_route_exists_and_preserves_release_marker():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/payment-confirmation-review" in actual_routes
