...
def test_assessment_factory_lite_payment_confirmation_event_release_marker_preserves_version_endpoint():
...
def test_assessment_factory_lite_payment_confirmation_event_release_marker_preserves_endpoint_route():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/payment-confirmation-event" in actual_routes


def test_assessment_factory_lite_payment_confirmation_event_release_marker_matches_service_contract():
...
def test_assessment_factory_lite_payment_confirmation_event_release_marker_matches_endpoint_contract():
...
def test_assessment_factory_lite_payment_confirmation_event_release_marker_preserves_source_layering():
    result = build_confirmed_event()

    assert result["source_payment_confirmation_review"]["release"] == (
        "assessment-factory-lite-scope-call-conversion"
    )
    assert result["source_payment_confirmation_review"]["version"] == "2.3.0"
    assert result["source_payment_confirmation_review"]["review_status"] == (
        "ready_for_payment_confirmation"
    )

    assert result["payment_evidence_review"]["payment_evidence_ready"] is True
    assert result["reconciliation_review"]["reconciliation_ready"] is True
    assert result["payment_request_record"]["payment_requested"] is True
    assert result["payment_request_delivery_record"][
        "payment_request_delivered_to_buyer"
    ] is True


def test_assessment_factory_lite_payment_confirmation_event_release_marker_preserves_payment_confirmation_and_downstream_boundaries():
    result = build_confirmed_event()

    assert result["payment_confirmation_record"]["payment_confirmed"] is True
    assert result["payment_confirmation_record"][
        "payment_confirmation_reference_recorded"
    ] is True
    assert result["payment_confirmation_record"]["payment_confirmed_at_recorded"] is True
    assert result["payment_confirmation_record"]["confirmed_amount_recorded"] is True
    assert result["payment_confirmation_record"][
    ...
    assert result["reconciliation_record"]["reconciliation_recorded"] is True
    ...
    assert result["operator_confirmation"]["human_operator_confirmed_payment"] is True
