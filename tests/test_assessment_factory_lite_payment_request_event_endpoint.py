...
def test_assessment_factory_lite_payment_request_event_endpoint_builds_contract():
    ...
    assert payload["payment_request_event_id"] == "payment-request-event-001"
    assert payload["recorded_at"] == "2026-08-01T12:00:00+00:00"
    assert payload["recommended_action"] == "prepare_payment_confirmation_review"


def test_assessment_factory_lite_payment_request_event_endpoint_summarizes_source_review():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
    ...
    assert payload["source_payment_request_review"] == {
        "event_type": "assessment_factory_lite_payment_request_review",
        "review_stage": "payment_request_review",
        "review_status": "ready_for_payment_request",
        "release": "assessment-factory-lite-scope-call-conversion",
        "version": "2.3.0",
        "payment_request_review_id": "payment-request-review-001",
        "prepared_at": "2026-07-31T12:00:00+00:00",
        "recommended_action": "prepare_payment_request_event",
    }


def test_assessment_factory_lite_payment_request_event_endpoint_tracks_payment_request_record():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
    ...
    assert payload["payment_request_record"] == {
        "payment_requested": True,
        "payment_request_reference": "payment-request-ref-001",
        "payment_requested_at": "2026-08-01T11:45:00+00:00",
        "requested_amount": "2500.00",
        "payment_request_reference_recorded": True,
        "payment_requested_at_recorded": True,
        "requested_amount_recorded": True,
        "payment_request_record_is_not_payment_confirmation": True,
        "payment_request_record_is_not_paid_work_authorization": True,
    }


def test_assessment_factory_lite_payment_request_event_endpoint_tracks_delivery_record():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_request_event_endpoint_tracks_operator_confirmation_without_confirmation_or_work_authorization():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_request_event_endpoint_builds_checklist_and_score():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
    ...
    assert payload["event_checklist"] == {
        "payment_request_review_ready": True,
        "payment_requested": True,
        "payment_request_reference_recorded": True,
        "payment_requested_at_recorded": True,
        "requested_amount_recorded": True,
...
def test_assessment_factory_lite_payment_request_event_endpoint_preserves_boundaries():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
    ...
    assert "payment_request_reference" in payload["evidence_boundary"]["allowed_evidence"]
...
def test_assessment_factory_lite_payment_request_event_endpoint_includes_audit_next_action_and_message():
    response = client.post(
        "/products/assessment-factory-lite/payment-request-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_request_event_endpoint_handles_pending_states():
    no_request = ready_payload()
    no_request["payment_request_event_context"] = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_request["payment_request_event_context"]["payment_requested"] = False
    ...
    assert payload["event_status"] == "pending_payment_request_confirmation"
    assert "payment_requested" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "confirm_payment_request"

    no_record = ready_payload()
    no_record["payment_request_event_context"] = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_record["payment_request_event_context"]["payment_request_reference"] = (
        "not_recorded"
    )
    ...
    assert payload["event_status"] == "pending_payment_request_record"
    assert "payment_request_reference_recorded" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "record_payment_request_reference"

    no_delivery = ready_payload()
    no_delivery["payment_request_event_context"] = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_delivery["payment_request_event_context"][
    ...
    assert payload["event_status"] == "pending_payment_request_delivery"
    assert "payment_request_delivered_to_buyer" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "record_payment_request_delivery"

    no_operator = ready_payload()
    no_operator["payment_request_event_context"] = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_operator["payment_request_event_context"][
    ...
    assert "human_operator_confirmed_payment_request" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "confirm_operator_payment_request_event"


def test_assessment_factory_lite_payment_request_event_endpoint_blocks_invalid_upstream_export():
    ...
    assert payload["source_payment_request_review"]["review_status"] == "blocked"
    assert payload["recommended_action"] == "resolve_payment_request_event_gaps"


def test_assessment_factory_lite_payment_request_event_endpoint_route_exists_and_preserves_release_marker():
