...
def test_assessment_factory_lite_payment_confirmation_review_release_marker_preserves_version_endpoint():
...
def test_assessment_factory_lite_payment_confirmation_review_release_marker_preserves_endpoint_route():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/payment-confirmation-review" in actual_routes


def test_assessment_factory_lite_payment_confirmation_review_release_marker_matches_service_contract():
...
def test_assessment_factory_lite_payment_confirmation_review_release_marker_matches_endpoint_contract():
...
def test_assessment_factory_lite_payment_confirmation_review_release_marker_preserves_source_layering():
    result = build_ready_review()

    assert result["source_payment_request_event"]["release"] == (
        "assessment-factory-lite-scope-call-conversion"
    )
    assert result["source_payment_request_event"]["version"] == "2.3.0"
    assert result["source_payment_request_event"]["event_status"] == "payment_requested"
...
def test_assessment_factory_lite_payment_confirmation_review_release_marker_preserves_payment_confirmation_and_downstream_boundaries():
    result = build_ready_review()
    ...
    assert result["operator_review"]["payment_confirmation_reviewed_by_operator"] is True
    assert result["operator_review"]["payment_confirmed"] is False
