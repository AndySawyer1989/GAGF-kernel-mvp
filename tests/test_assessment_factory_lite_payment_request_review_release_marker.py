...
def test_assessment_factory_lite_payment_request_review_release_marker_preserves_version_endpoint():
...
def test_assessment_factory_lite_payment_request_review_release_marker_preserves_endpoint_route():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/payment-request-review" in actual_routes


def test_assessment_factory_lite_payment_request_review_release_marker_matches_service_contract():
...
def test_assessment_factory_lite_payment_request_review_release_marker_matches_endpoint_contract():
...
def test_assessment_factory_lite_payment_request_review_release_marker_preserves_source_layering():
    result = build_ready_review()

    assert result["source_invoice_creation_event"]["release"] == (
        "assessment-factory-lite-scope-call-conversion"
    )
    assert result["source_invoice_creation_event"]["version"] == "2.3.0"
    assert result["source_invoice_creation_event"]["event_status"] == "invoice_created"

    assert result["invoice_record"]["invoice_created"] is True
    assert result["invoice_record"]["invoice_reference_recorded"] is True
    assert result["delivery_record"]["invoice_delivered_to_buyer"] is True
    assert result["invoice_details_review"]["invoice_details_ready"] is True
    assert result["billing_readiness"]["billing_ready"] is True


def test_assessment_factory_lite_payment_request_review_release_marker_preserves_payment_request_and_downstream_boundaries():
    result = build_ready_review()
    ...
    assert result["operator_review"]["payment_request_reviewed_by_operator"] is True
    assert result["operator_review"]["payment_requested"] is False
