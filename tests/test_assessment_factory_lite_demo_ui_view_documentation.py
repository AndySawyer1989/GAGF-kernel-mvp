...
DOC_PATH = Path("docs/ASSESSMENT_FACTORY_LITE_DEMO_UI_VIEW.md")


def test_assessment_factory_lite_demo_ui_view_document_exists():
    assert DOC_PATH.exists()


def test_assessment_factory_lite_demo_ui_view_document_names_service_and_endpoint():
...
def test_assessment_factory_lite_demo_ui_view_document_names_input_modes():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_ui_view_document_names_output_contract():
    content = DOC_PATH.read_text(encoding="utf-8")
    ...
    assert "ui_sections" in content
    assert "cards" in content
    assert "operator_actions" in content
    assert "warnings" in content
    assert "data_boundary" in content
    assert "source_payloads" in content
    assert "operator_message" in content
    assert "recommended_action" in content


def test_assessment_factory_lite_demo_ui_view_document_names_sections_and_cards():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_ui_view_document_names_operator_actions():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_ui_view_document_names_warnings_and_boundary():
    content = DOC_PATH.read_text(encoding="utf-8")
...
def test_assessment_factory_lite_demo_ui_view_document_preserves_boundaries():
    content = DOC_PATH.read_text(encoding="utf-8")

    assert (
        "The Assessment Factory Lite Demo UI View does not certify products "
