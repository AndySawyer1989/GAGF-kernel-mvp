...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_builds_contract():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
    ...
    assert payload["event_type"] == "assessment_factory_lite_payment_confirmation_event"
    ...
    assert payload["event_stage"] == "payment_confirmation_event"
    assert payload["event_status"] == "payment_confirmed"
    assert payload["payment_confirmation_event_id"] == "payment-confirmation-event-001"
    assert payload["recorded_at"] == "2026-08-03T12:00:00+00:00"
    assert payload["recommended_action"] == "prepare_paid_assessment_authorization_review"


def test_assessment_factory_lite_payment_confirmation_event_endpoint_summarizes_source_review():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
    ...
    assert payload["source_payment_confirmation_review"] == {
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_tracks_payment_confirmation_record():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
    ...
    assert payload["payment_confirmation_record"] == {
        "payment_confirmed": True,
        "payment_confirmation_reference": "payment-confirmation-ref-001",
        "payment_confirmed_at": "2026-08-03T11:45:00+00:00",
        "confirmed_amount": "2500.00",
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_tracks_reconciliation_record():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
    ...
    assert payload["reconciliation_record"] == {
        "amount_reconciled": True,
        "invoice_reference_reconciled": True,
        "payment_method_recorded": True,
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_tracks_operator_confirmation_without_work_authorization():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_builds_checklist_and_score():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_preserves_boundaries():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
    ...
    assert "payment_confirmation_reference" in payload["evidence_boundary"]["allowed_evidence"]
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_includes_audit_next_action_and_message():
    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=ready_payload(),
    )
...
def test_assessment_factory_lite_payment_confirmation_event_endpoint_handles_pending_states():
    no_confirmation = ready_payload()
    no_confirmation["payment_confirmation_event_context"] = dict(
        PAYMENT_CONFIRMATION_EVENT_CONTEXT
    )
    no_confirmation["payment_confirmation_event_context"]["payment_confirmed"] = False
    ...
    assert payload["event_status"] == "pending_payment_confirmation"
    assert "payment_confirmed" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "confirm_payment_received"

    no_record = ready_payload()
    no_record["payment_confirmation_event_context"] = dict(
        PAYMENT_CONFIRMATION_EVENT_CONTEXT
    )
    no_record["payment_confirmation_event_context"][
    ...
    assert payload["event_status"] == "pending_payment_confirmation_record"
    assert "payment_confirmation_reference_recorded" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "record_payment_confirmation_reference"

    no_reconciliation = ready_payload()
    no_reconciliation["payment_confirmation_event_context"] = dict(
        PAYMENT_CONFIRMATION_EVENT_CONTEXT
    )
    no_reconciliation["payment_confirmation_event_context"]["amount_reconciled"] = False

    response = client.post(
        "/products/assessment-factory-lite/payment-confirmation-event",
        json=no_reconciliation,
    )
    ...
    assert payload["event_status"] == "pending_reconciliation_record"
    assert "reconciliation_recorded" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "record_payment_reconciliation"

    no_operator = ready_payload()
    no_operator["payment_confirmation_event_context"] = dict(
        PAYMENT_CONFIRMATION_EVENT_CONTEXT
    )
    no_operator["payment_confirmation_event_context"][
    ...
    assert "human_operator_confirmed_payment" in payload["event_blockers"]
    assert payload["next_action"]["action"] == "confirm_operator_payment_confirmation_event"


def test_assessment_factory_lite_payment_confirmation_event_endpoint_blocks_invalid_upstream_export():
    ...
    assert payload["source_payment_confirmation_review"]["review_status"] == "blocked"
    assert payload["recommended_action"] == "resolve_payment_confirmation_event_gaps"


def test_assessment_factory_lite_payment_confirmation_event_endpoint_route_exists_and_preserves_release_marker():
    actual_routes = {route.path for route in app.routes}

    assert "/products/assessment-factory-lite/payment-confirmation-event" in actual_routes
