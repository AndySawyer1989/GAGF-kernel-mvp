...
def test_assessment_factory_lite_payment_request_event_service_builds_contract():
    ...
    assert result["payment_request_event_id"] == "payment-request-event-001"
    assert result["recorded_at"] == "2026-08-01T12:00:00+00:00"
    assert result["recommended_action"] == "prepare_payment_confirmation_review"


def test_assessment_factory_lite_payment_request_event_service_summarizes_source_review():
    result = build_requested_event()

    assert result["source_payment_request_review"] == {
...
def test_assessment_factory_lite_payment_request_event_service_tracks_payment_request_record():
    result = build_requested_event()
...
def test_assessment_factory_lite_payment_request_event_service_tracks_delivery_record():
    result = build_requested_event()
...
def test_assessment_factory_lite_payment_request_event_service_tracks_operator_confirmation_without_confirmation_or_work_authorization():
    result = build_requested_event()
...
def test_assessment_factory_lite_payment_request_event_service_builds_checklist_and_score():
    result = build_requested_event()
...
def test_assessment_factory_lite_payment_request_event_service_preserves_boundaries():
    result = build_requested_event()
    ...
    assert "payment_request_reference" in result["evidence_boundary"]["allowed_evidence"]
...
def test_assessment_factory_lite_payment_request_event_service_includes_audit_next_action_and_message():
    result = build_requested_event()
...
def test_assessment_factory_lite_payment_request_event_service_handles_pending_states():
    no_request = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_request["payment_requested"] = False

    result = AssessmentFactoryLitePaymentRequestEventService().record_event(
        operator_approval=APPROVAL,
        event_context=EVENT_CONTEXT,
        follow_up_context=INTERESTED_CONTEXT,
        follow_up_event_context=FOLLOW_UP_EVENT_CONTEXT,
        ...
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=PAYMENT_REQUEST_CONTEXT,
        payment_request_event_context=no_request,
    )

    assert result["event_status"] == "pending_payment_request_confirmation"
    assert "payment_requested" in result["event_blockers"]
    assert result["next_action"]["action"] == "confirm_payment_request"

    no_record = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_record["payment_request_reference"] = "not_recorded"
    ...
    result = AssessmentFactoryLitePaymentRequestEventService().record_event(
        ...
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=PAYMENT_REQUEST_CONTEXT,
        payment_request_event_context=no_record,
    )

    assert result["event_status"] == "pending_payment_request_record"
    assert "payment_request_reference_recorded" in result["event_blockers"]
    assert result["next_action"]["action"] == "record_payment_request_reference"

    no_delivery = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_delivery["payment_request_delivered_to_buyer"] = False
    ...
    result = AssessmentFactoryLitePaymentRequestEventService().record_event(
        ...
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=PAYMENT_REQUEST_CONTEXT,
        payment_request_event_context=no_delivery,
    )

    assert result["event_status"] == "pending_payment_request_delivery"
    assert "payment_request_delivered_to_buyer" in result["event_blockers"]
    assert result["next_action"]["action"] == "record_payment_request_delivery"

    no_operator = dict(PAYMENT_REQUEST_EVENT_CONTEXT)
    no_operator["human_operator_confirmed_payment_request"] = False
    ...
    result = AssessmentFactoryLitePaymentRequestEventService().record_event(
        ...
        invoice_creation_context=INVOICE_CREATION_CONTEXT,
        payment_request_context=PAYMENT_REQUEST_CONTEXT,
        payment_request_event_context=no_operator,
    )

    assert result["event_status"] == "pending_operator_confirmation"
    assert "human_operator_confirmed_payment_request" in result["event_blockers"]
    assert result["next_action"]["action"] == "confirm_operator_payment_request_event"


def test_assessment_factory_lite_payment_request_event_service_blocks_invalid_upstream_export():
    ...
    assert result["source_payment_request_review"]["review_status"] == "blocked"
    assert result["recommended_action"] == "resolve_payment_request_event_gaps"